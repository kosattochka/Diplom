<footer>
    <div>
        <div class="footer-content">
            <div class="column">
                <div class="pank">
                    <img src="/img/footer-icon-phone.svg" alt="">
                    <a href="tel:{{$phone}}">Офис продаж (звонок бесплатный) {{$phone}}</a>
                </div>
                <div>
                    <img src="/img/footer-icon-telegram.svg" alt="">
                    <a href="{{$telegram}}">Телеграм</a>
                </div>
            </div>
            <div class="column">
                <div>
                    <img src="/img/footer-icon-vk.svg" alt="">
                    <a href="{{$vk}}">Вконтакте</a>
                </div>
                <div class="pank">
                    <img src="/img/footer-icon-email.svg" alt="">
                    <a href="mailto:{{$email}}">E-mail: {{$email}}</a>
                </div>
            </div>
        </div>
        <div class="footer-info">
            <p>Адрес офиса продаж: {{$address}}</p>
            <p class="pank">На нашем сайте есть <a href="/politics">политика конфиденциальности.</a></p>
            <p class="pank">© 2009-2025 Все права защищены Круглогодичный центр отдыха "Павловский парк". Официальный сайт</p>
        </div>
    </div>
    <div>
        <img src="/img/logo.svg" alt="">
    </div>
</footer>
